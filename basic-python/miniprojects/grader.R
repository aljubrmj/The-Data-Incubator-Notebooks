...
test_cases_grading <- function(question_name, func, test_cases) {
    ...
    payload <- toJSON(list(question_name = question_name, submission = sub, kernel_history = list()), dataframe = "values", factors = "string", auto_unbox = TRUE)
    ...
    score <- tryCatch({
        ...
        print("There was an error.  Please show this output to a TDI staff member.")
        print(err)
        return(NULL)
    })
    if (!is.null(score)) {
